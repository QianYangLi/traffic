...
class LayerParams:
    ...
    def get_weights(self, shape):
        if shape not in self._params_dict:
            ...
            self._rnn_network.register_parameter(
                f"{self._type}_weight_{str(shape)}", nn_param
            )
    ...
    def get_biases(self, length, bias_start=0.0):
        if length not in self._biases_dict:
            ...
            self._rnn_network.register_parameter(
                f"{self._type}_biases_{str(length)}", biases
            )
...
class DCGRUCell(nn.Module):
    ...
    def _gconv(self, inputs, state, output_size, bias_start=0.0):
        ...
        x = inputs_and_state
        x0 = x.permute(1, 2, 0)  # (num_nodes, input_size, batch)
        ...
        x = x.permute(3, 1, 2, 0)  # (batch, num_nodes, input_size, order)
        x = torch.reshape(
...
class EncoderModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, adj_mx, seq_len, nodes, device):
    ...
    def forward(self, inputs, hidden_state=None):
        """
        inputs: [B, num_nodes]
        hidden_state: [num_layers, B, hidden_state_size]
        """
...
class DecoderModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, adj_mx, nodes=529, horizon=1, device="cuda:0"):
        nn.Module.__init__(self)
        Seq2SeqAttrs.__init__(self, adj_mx, nodes)
    ...
    def forward(self, inputs, hidden_state=None):
        """
        inputs: [B, num_nodes * output_dim] = [B, num_nodes]
        hidden_state: [num_layers, B, hidden_state_size]
        """
        hidden_states = []
        output = inputs
...
class DCRNNModel(nn.Module, Seq2SeqAttrs):
    ...
    def decoder(self, encoder_hidden_state, labels=None, batches_seen=None):
        """
        encoder_hidden_state: [num_layers, B, hidden_state_size]
        labels: [horizon, B, num_nodes]
        """
        ...
        outputs = torch.stack(outputs)   # [horizon, B, num_nodes]
        return outputs

    def forward(self, inputs, labels=None, batches_seen=None):
        """
        inputs: [B, seq_len, num_nodes]
        labels: [B, horizon, num_nodes]
        return: [B, horizon, num_nodes]
        """
        ...
        outputs = self.decoder(
            encoder_hidden_state, labels, batches_seen=batches_seen
        )
